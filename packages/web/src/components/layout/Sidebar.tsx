	...
	return (
		<AnimatePresence>
			...
			<motion.aside
				...
				animate={{ x: mobileOpen ? 0 : -20, opacity: 1 }}
				exit={{ x: -20, opacity: 0 }}
				transition={{ duration: 0.3, ease: "easeOut" }}
				className="w-14 sm:w-56 lg:w-56 shrink-0 flex flex-col h-full fixed lg:static z-50"
				style={{
					background: "var(--sidebar-bg)",
					borderRight: "1px solid var(--border)",
					position: "relative",
					transform: mobileOpen ? "translateX(0)" : "translateX(-100%)",
					boxShadow: mobileOpen ? "0 24px 48px rgba(0,0,0,0.4)" : "none",
				}}
			>
				{/* Logo */}
